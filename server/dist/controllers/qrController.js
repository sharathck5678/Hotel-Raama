...
class QrController {
    ...
    static async createOrder(req, res) {
        try {
            ...
            const validObjectIds = items
                .map((i) => i.menuItemId || i._id)
                .filter((id) => id && mongoose_1.Types.ObjectId.isValid(id));
            let menuItems = [];
            if (validObjectIds.length > 0) {
                menuItems = await MenuItem_1.MenuItem.find({ _id: { $in: validObjectIds } });
            }
            ...
            if (chosenPaymentMethod === 'RAZORPAY') {
                const razorpayOrder = await RazorpayService_1.RazorpayService.createOrder(totalAmount, orderId);
    ...
    static async verifyPayment(req, res) {
        try {
            ...
            // Verify signature
            const isValid = RazorpayService_1.RazorpayService.verifyPaymentSignature(razorpayOrderId, razorpayPaymentId, razorpaySignature || 'mock_sig');
            if (!isValid) {
