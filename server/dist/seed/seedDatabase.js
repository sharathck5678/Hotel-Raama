...
const seed = async () => {
    try {
        ...
        const roomTypesData = [
            { name: 'Premium Single Non A/C', code: 'PREM_SGL_NONAC', description: 'Comfortable single occupancy non-A/C room with queen bed, Wi-Fi, and city views.', basePrice: 1200, cpPrice: 1350, maxOccupancy: 1, isAc: false, amenities: ['Free Wi-Fi', 'TV', 'Hot Water', 'Work Desk'], images: ['/single-occupancy-room.png'] },
            { name: 'Premium Double Non A/C', code: 'PREM_DBL_NONAC', description: 'Spacious double occupancy non-A/C room with plush bedding and modern bathroom.', basePrice: 1600, cpPrice: 1800, maxOccupancy: 2, isAc: false, amenities: ['Free Wi-Fi', 'LED TV', '24/7 Hot Water', 'Daily Housekeeping'], images: ['/double-occupancy-room.png'] },
            { name: 'Executive Single A/C', code: 'EXEC_SGL_AC', description: 'Elegant single room with climate control A/C, ergonomic desk, and premium bath accessories.', basePrice: 1800, cpPrice: 2000, maxOccupancy: 1, isAc: true, amenities: ['Air Conditioning', 'High Speed Wi-Fi', 'Smart TV', 'Room Service'], images: ['/single-occupancy-room.png'] },
            { name: 'Executive Double A/C', code: 'EXEC_DBL_AC', description: 'Luxurious double A/C room equipped with king-size bed, seating area, and room dining.', basePrice: 2200, cpPrice: 2500, maxOccupancy: 2, isAc: true, amenities: ['Air Conditioning', 'King Bed', 'Tea/Coffee Maker', 'Minibar', 'Smart TV'], images: ['/double-occupancy-room.png'] },
            { name: 'Triple Occupancy Premium', code: 'TRIPLE_PREM', description: 'Generous room designed for families or small groups with 3 comfortable single beds.', basePrice: 2400, cpPrice: 2750, maxOccupancy: 3, isAc: false, amenities: ['3 Single Beds', 'Free Wi-Fi', 'Spacious Wardrobe', 'Bottled Water'], images: ['/triple-occupancy-ac.png'] },
            { name: 'Triple Occupancy Executive A/C', code: 'TRIPLE_EXEC', description: 'Air-conditioned family room featuring premium bedding, extra seating, and deluxe amenities.', basePrice: 2800, cpPrice: 3200, maxOccupancy: 3, isAc: true, amenities: ['Air Conditioning', '3 Beds', 'Smart TV', 'Tea/Coffee Station'], images: ['/triple-occupancy-ac.png'] },
            { name: 'Suite Room', code: 'SUITE_ROOM', description: 'Presidential suite with separate living lounge, master bedroom, luxury bathtub, and VIP service.', basePrice: 3500, cpPrice: 4000, maxOccupancy: 4, isAc: true, amenities: ['Living Room Lounge', 'Jacuzzi / Bathtub', 'Fruit Basket', 'Express Check-in', 'Premium A/C'], images: ['/suite-room.png'] },
        ];
        ...
        const catSouth = await MenuCategory_1.MenuCategory.create({ name: 'South Indian Dishes (ದಕ್ಷಿಣ ಭಾರತೀಯ ತಿನಿಸುಗಳು)', section: 'SWAAD', description: 'Timings: 7:00 AM to 11:30 AM', sortOrder: 1 });
        const catDosa = await MenuCategory_1.MenuCategory.create({ name: 'Dosa Specialities (ದೋಸೆ ಸ್ಪೆಷಲ್)', section: 'SWAAD', description: 'Timings: 7:00 AM to 11:30 AM & 3:30 PM to 8:30 PM', sortOrder: 2 });
        const catMeals = await MenuCategory_1.MenuCategory.create({ name: 'Meals (ಮೀಲ್ಸ್)', section: 'SWAAD', description: 'South Indian & North Indian Meals', sortOrder: 3 });
        const catNorth = await MenuCategory_1.MenuCategory.create({ name: 'North Pulav & Biriyani (ನಾರ್ತ್ ಪಲಾವ್ & ಬಿರಿಯಾನಿ)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 4 });
        const catStarters = await MenuCategory_1.MenuCategory.create({ name: 'Starters (ಸ್ಟಾರ್ಟರ್ಸ್)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 5 });
        const catSoups = await MenuCategory_1.MenuCategory.create({ name: 'Soups (ಸೂಪ್)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 6 });
        const catSalad = await MenuCategory_1.MenuCategory.create({ name: 'Salad (ಸಲಾಡ್)', section: 'SWAAD', sortOrder: 7 });
        const catBreads = await MenuCategory_1.MenuCategory.create({ name: 'Tandoor Bread (ತಂದೂರ್ ಬ್ರೆಡ್)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 8 });
        const catCurry = await MenuCategory_1.MenuCategory.create({ name: 'Vegetable Curry (ವೆಜಿಟೆಬಲ್ ಕರಿ)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 9 });
        const catKadai = await MenuCategory_1.MenuCategory.create({ name: 'Kadai Special (ಕಡೈ ಸ್ಪೆಷಲ್)', section: 'SWAAD', sortOrder: 10 });
        const catKofta = await MenuCategory_1.MenuCategory.create({ name: 'Kofta & Stuffed (ಕೋಫ್ತಾ & ಸ್ಟಫ್ಡ್)', section: 'SWAAD', sortOrder: 11 });
        const catDalPalak = await MenuCategory_1.MenuCategory.create({ name: 'Dal & Palak (ದಾಲ್ ಪಾಲಕ್)', section: 'SWAAD', sortOrder: 12 });
        const catChinese = await MenuCategory_1.MenuCategory.create({ name: 'Chinese Rice & Noodles (ಚೈನೀಸ್ ರೈಸ್ & ನೂಡಲ್ಸ್)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 13 });
        const catSandwich = await MenuCategory_1.MenuCategory.create({ name: 'Sandwich & Grilled (ಸ್ಯಾಂಡ್‌ವಿಚ್ & ಗ್ರಿಲ್ಡ್)', section: 'SWAAD', sortOrder: 14 });
        const catSweets = await MenuCategory_1.MenuCategory.create({ name: 'Sweet Special (ಸ್ವೀಟ್ ಸ್ಪೆಷಲ್)', section: 'SWAAD', sortOrder: 15 });
        const catJuice = await MenuCategory_1.MenuCategory.create({ name: 'Fresh Fruit Juice (ಫ್ರೆಶ್ ಫ್ರೂಟ್ ಜ್ಯೂಸ್)', section: 'SWAAD', description: 'Timings: 11:30 AM to 10:00 PM', sortOrder: 16 });
        const catShakes = await MenuCategory_1.MenuCategory.create({ name: 'Milkshakes & Lassi (ಮಿಲ್ಕ್ ಶೇಕ್ & ಲಸಿ)', section: 'SWAAD', sortOrder: 17 });
        const catFruitSalad = await MenuCategory_1.MenuCategory.create({ name: 'Fruit Salad (ಫ್ರೂಟ್ ಸಲಾಡ್)', section: 'SWAAD', sortOrder: 18 });
        const catIceCream = await MenuCategory_1.MenuCategory.create({ name: 'Ice Cream Scoop & Cone (ಐಸ್ ಕ್ರೀಮ್ ಸ್ಕೂಪ್ & ಕೋನ್)', section: 'SWAAD', sortOrder: 19 });
        const catSplIceCream = await MenuCategory_1.MenuCategory.create({ name: 'Special Ice Cream (ಸ್ಪೆಷಲ್ ಐಸ್ ಕ್ರೀಮ್)', section: 'SWAAD', sortOrder: 20 });
        const catHotBev = await MenuCategory_1.MenuCategory.create({ name: 'Hot Beverages (ಬಿಸಿಯಾದ ಪಾನೀಯ)', section: 'SWAAD', sortOrder: 21 });
        const swaadItems = [
        ...
        const catLlbWhisky = await MenuCategory_1.MenuCategory.create({ name: 'Premium Whisky (ಪ್ರೀಮಿಯಂ ವಿಸ್ಕಿ)', section: 'LIQUID_LOUNGE', sortOrder: 1 });
        const catLlbBrandy = await MenuCategory_1.MenuCategory.create({ name: 'Brandy (ಬ್ರಾಂಡಿ)', section: 'LIQUID_LOUNGE', sortOrder: 2 });
        const catLlbRum = await MenuCategory_1.MenuCategory.create({ name: 'Rum (ರಮ್)', section: 'LIQUID_LOUNGE', sortOrder: 3 });
        const catLlbVodka = await MenuCategory_1.MenuCategory.create({ name: 'Vodka (ವೋಡ್ಕಾ)', section: 'LIQUID_LOUNGE', sortOrder: 4 });
        const catLlbScotch = await MenuCategory_1.MenuCategory.create({ name: 'Scotch & Single Malt (ಸ್ಕಾಚ್ & ಸಿಂಗಲ್ ಮಾಲ್ಟ್ ವಿಸ್ಕಿ)', section: 'LIQUID_LOUNGE', sortOrder: 5 });
        const catLlbWine = await MenuCategory_1.MenuCategory.create({ name: 'Wine (ವೈನ್)', section: 'LIQUID_LOUNGE', sortOrder: 6 });
        const catLlbTequila = await MenuCategory_1.MenuCategory.create({ name: 'Tequila (ಟೆಕಿಲಾ)', section: 'LIQUID_LOUNGE', sortOrder: 7 });
        const catLlbBeer = await MenuCategory_1.MenuCategory.create({ name: 'Beer (ಬೀರ್)', section: 'LIQUID_LOUNGE', sortOrder: 8 });
        const catLlbBreezer = await MenuCategory_1.MenuCategory.create({ name: 'Breezer (ಬ್ರೀಜರ್)', section: 'LIQUID_LOUNGE', sortOrder: 9 });
        const catLlbMocktail = await MenuCategory_1.MenuCategory.create({ name: 'Mocktails (ಮಾಕ್‌ಟೇಲ್)', section: 'LIQUID_LOUNGE', sortOrder: 10 });
        const catLlbCocktail = await MenuCategory_1.MenuCategory.create({ name: 'Cocktails (ಕಾಕ್‌ಟೇಲ್)', section: 'LIQUID_LOUNGE', sortOrder: 11 });
        const catLlbBeverages = await MenuCategory_1.MenuCategory.create({ name: 'Water & Soft Drinks (ನೀರು ಮತ್ತು ತಂಪು ಪಾನೀಯಗಳು)', section: 'LIQUID_LOUNGE', sortOrder: 12 });
        const llbItems = [
        ...
        const catHrEgg = await MenuCategory_1.MenuCategory.create({ name: 'Egg Specialities (ಎಗ್ / ಮೊಟ್ಟೆ ತಿನಿಸುಗಳು)', section: 'HOTEL_RAAMA', sortOrder: 1 });
        const catHrLittleBite = await MenuCategory_1.MenuCategory.create({ name: 'Little Bite (ಲಿಟಲ್ ಬೈಟ್ / ತಿಂಡಿಗಳು)', section: 'HOTEL_RAAMA', sortOrder: 2 });
        const catHrVegs = await MenuCategory_1.MenuCategory.create({ name: 'Vegetables (ವೆಜಿಟೇಬಲ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 3 });
        const catHrNonVegSoup = await MenuCategory_1.MenuCategory.create({ name: 'Non Veg Soup (ನಾನ್‌ವೆಜ್ ಸೂಪ್)', section: 'HOTEL_RAAMA', sortOrder: 4 });
        const catHrSizzler = await MenuCategory_1.MenuCategory.create({ name: 'Sizzler Special (ಸಿಜ್ಲರ್ ಸ್ಪೆಷಲ್)', section: 'HOTEL_RAAMA', sortOrder: 5 });
        const catHrVegSoup = await MenuCategory_1.MenuCategory.create({ name: 'Veg Soups (ವೆಜ್ ಸೂಪ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 6 });
        const catHrTandooriNonVeg = await MenuCategory_1.MenuCategory.create({ name: 'Tandoori Non Veg (ತಂದೂರಿ ನಾನ್‌ವೆಜ್)', section: 'HOTEL_RAAMA', sortOrder: 7 });
        const catHrChickenCurry = await MenuCategory_1.MenuCategory.create({ name: 'Chicken Special Curry (ಚಿಕನ್ ಸ್ಪೆಷಲ್ ಕರಿ)', section: 'HOTEL_RAAMA', sortOrder: 8 });
        const catHrChineseBites = await MenuCategory_1.MenuCategory.create({ name: 'Chinese & Indian Bites (ಚೈನೀಸ್ & ಇಂಡಿಯನ್ ಬೈಟ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 9 });
        const catHrMuttonSpecial = await MenuCategory_1.MenuCategory.create({ name: 'Mutton Special (ಮಟನ್ ಸ್ಪೆಷಲ್)', section: 'HOTEL_RAAMA', sortOrder: 10 });
        const catHrMuttonMain = await MenuCategory_1.MenuCategory.create({ name: 'Mutton Main Course (ಮಟನ್ ಮೇನ್ ಕೋರ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 11 });
        const catHrBiriyani = await MenuCategory_1.MenuCategory.create({ name: 'Biriyani (ಬಿರಿಯಾನಿ)', section: 'HOTEL_RAAMA', sortOrder: 12 });
        const catHrRiceNonVeg = await MenuCategory_1.MenuCategory.create({ name: 'Rice & Noodles (Non-Veg) (ರೈಸ್ & ನೂಡಲ್ಸ್ - ನಾನ್‌ವೆಜ್)', section: 'HOTEL_RAAMA', sortOrder: 13 });
        const catHrRiceVeg = await MenuCategory_1.MenuCategory.create({ name: 'Rice & Noodles (Veg) (ರೈಸ್ & ನೂಡಲ್ಸ್ - ವೆಜ್)', section: 'HOTEL_RAAMA', sortOrder: 14 });
        const catHrTandooriVeg = await MenuCategory_1.MenuCategory.create({ name: 'Tandoori (Veg) (ತಂದೂರಿ ವೆಜ್)', section: 'HOTEL_RAAMA', sortOrder: 15 });
        const catHrVegStarters = await MenuCategory_1.MenuCategory.create({ name: 'Veg Starters (ವೆಜ್ ಸ್ಟಾರ್ಟರ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 16 });
        const catHrVegCurry = await MenuCategory_1.MenuCategory.create({ name: 'Vegetable Curry (ವೆಜಿಟೇಬಲ್ ಕರಿ)', section: 'HOTEL_RAAMA', sortOrder: 17 });
        const catHrIndianBreads = await MenuCategory_1.MenuCategory.create({ name: 'Indian Breads (ಇಂಡಿಯನ್ ಬ್ರೆಡ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 18 });
        const catHrKadai = await MenuCategory_1.MenuCategory.create({ name: 'Kadai Special (ಕಡಾಯಿ ಸ್ಪೆಷಲ್)', section: 'HOTEL_RAAMA', sortOrder: 19 });
        const catHrSeaFood = await MenuCategory_1.MenuCategory.create({ name: 'Sea Food Seasonal (ಸೀ ಫುಡ್ ಸೀಸನಲ್)', section: 'HOTEL_RAAMA', sortOrder: 20 });
        const catHrSoftDrinks = await MenuCategory_1.MenuCategory.create({ name: 'Soft Drinks and More (ಸಾಫ್ಟ್ ಡ್ರಿಂಕ್ಸ್ ಮತ್ತು ಇತರೆ)', section: 'HOTEL_RAAMA', sortOrder: 21 });
        const catHrDal = await MenuCategory_1.MenuCategory.create({ name: 'Dal Special (ದಾಲ್ ಸ್ಪೆಷಲ್)', section: 'HOTEL_RAAMA', sortOrder: 22 });
        const catHrKofta = await MenuCategory_1.MenuCategory.create({ name: 'Kofta & Stuffed (ಕೋಫ್ತಾ ಮತ್ತು ಸ್ಟಫ್ಡ್)', section: 'HOTEL_RAAMA', sortOrder: 23 });
        const catHrJuices = await MenuCategory_1.MenuCategory.create({ name: 'Fresh Fruit Juices & Shakes (ಫ್ರೆಶ್ ಫ್ರೂಟ್ ಜ್ಯೂಸ್ & ಮಿಲ್ಕ್ ಶೇಕ್ಸ್)', section: 'HOTEL_RAAMA', sortOrder: 24 });
        const catHrFrozen = await MenuCategory_1.MenuCategory.create({ name: 'Frozen Treat (ಫ್ರೋಜನ್ ಟ್ರೀಟ್)', section: 'HOTEL_RAAMA', sortOrder: 25 });
        const catHrIceCream = await MenuCategory_1.MenuCategory.create({ name: 'Ice Creams Scoop (ಐಸ್ ಕ್ರೀಮ್ಸ್ ಸ್ಕೂಪ್)', section: 'HOTEL_RAAMA', sortOrder: 26 });
        ...
        await MenuItem_1.MenuItem.create(hotelRaamaItems);
