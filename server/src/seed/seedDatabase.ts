...
export const runSeedLogic = async (clearExisting = false) => {
  ...
  if (!existingSetting) {
    ...
    const catStarters = await MenuCategory.create({ name: 'Starters (ಸ್ಟಾರ್ಟರ್ಸ್)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 4 });
    const catSalad = await MenuCategory.create({ name: 'Salad (ಸಲಾಡ್)', section: 'SWAAD', sortOrder: 5 });
    const catBreads = await MenuCategory.create({ name: 'Tandoor Bread (ತಂದೂರ್ ಬ್ರೆಡ್)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 6 });
    const catCurry = await MenuCategory.create({ name: 'Vegetable Curry (ವೆಜಿಟೆಬಲ್ ಕರಿ)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 7 });
    const catKadai = await MenuCategory.create({ name: 'Kadai Special (ಕಡೈ ಸ್ಪೆಷಲ್)', section: 'SWAAD', sortOrder: 8 });
    const catKofta = await MenuCategory.create({ name: 'Kofta & Stuffed (ಕೋಫ್ತಾ & ಸ್ಟಫ್ಡ್)', section: 'SWAAD', sortOrder: 9 });
    const catDalPalak = await MenuCategory.create({ name: 'Dal & Palak (ದಾಲ್ ಪಾಲಕ್)', section: 'SWAAD', sortOrder: 10 });
    const catMeals = await MenuCategory.create({ name: 'Meals (ಮೀಲ್ಸ್)', section: 'SWAAD', description: 'South Indian & North Indian Meals', sortOrder: 11 });
    const catNorth = await MenuCategory.create({ name: 'North Pulav & Biriyani (ನಾರ್ತ್ ಪಲಾವ್ & ಬಿರಿಯಾನಿ)', section: 'SWAAD', description: 'Timings: 12:00 Noon to 3:30 PM & 7:00 PM to 10:00 PM', sortOrder: 12 });
    ...
    const catFruitSalad = await MenuCategory.create({ name: 'Fruit Salad (ಫ್ರೂಟ್ ಸಲಾಡ್)', section: 'SWAAD', sortOrder: 16 });
    const catIceCream = await MenuCategory.create({ name: 'Ice Cream Scoop & Cone (ಐಸ್ ಕ್ರೀಮ್ ಸ್ಕೂಪ್ & ಕೋನ್)', section: 'SWAAD', sortOrder: 17 });
    const catSplIceCream = await MenuCategory.create({ name: 'Special Ice Cream (ಸ್ಪೆಷಲ್ ಐಸ್ ಕ್ರೀಮ್)', section: 'SWAAD', sortOrder: 18 });
    const catJuice = await MenuCategory.create({ name: 'Fresh Fruit Juice (ಫ್ರೆಶ್ ಫ್ರೂಟ್ ಜ್ಯೂಸ್)', section: 'SWAAD', description: 'Timings: 11:30 AM to 10:00 PM', sortOrder: 19 });
    const catShakes = await MenuCategory.create({ name: 'Milkshakes & Lassi (ಮಿಲ್ಕ್ ಶೇಕ್ & ಲಸಿ)', section: 'SWAAD', sortOrder: 20 });
